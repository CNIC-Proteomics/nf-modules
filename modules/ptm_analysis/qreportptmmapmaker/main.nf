process QREPORT_PTMMAP_MAKER {
    ...
    source ${REPORTANALYSIS_HOME}/env/bin/activate && python ${REPORTANALYSIS_HOME}/src/PTMMap.py  -i "${input_file}" -c "${params_file}" -o "." 2>&1

    source ${REPORTANALYSIS_HOME}/env/bin/activate && python ${REPORTANALYSIS_HOME}/src/qReportMaker.py  -i "${input_file}" -c "${params_file}" -p "." ${optional_params} -o "." 2>&1
    """
}
