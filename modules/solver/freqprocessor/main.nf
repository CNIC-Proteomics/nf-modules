process FREQ_PROCESSOR {
    ...
    def out_pgm  = "${input_file.baseName}_PDM_Table_pgmFreq.tsv"
    def out_pgm  = "${input_file.baseName}_PGM_Table_pgmFreq.tsv"

    """
    source ${PTMCOMPASS_HOME}/env/bin/activate && python ${SOLVER_HOME}/FreqProcessor.py -i "${input_file}" -od "${out_pgm}" -og "${out_pgm}" > "${log_file}" 2>&1
    """
}
