...
workflow CREATE_INPUT_CHANNEL_PTM_ANALYSIS {
    ...
    ch_quant_file = Channel.fromPath("${params.quant_file}", checkIfExists: true)
    ch_pgm_file = Channel.fromPath("${params.pgm_file}", checkIfExists: true)
    ch_compa_file = Channel.fromPath("${params.compa_file}", checkIfExists: true)
    ...
    ch_params_file = Channel.fromPath("${updated_params_file}", checkIfExists: true)

    // create channels from input files
    // this file will be used multiple times, so, it is why the variable changes to file
    ch_qmeta_file = Channel.fromPath("${params.NO_FILE}", checkIfExists: true)
    if ( params.containsKey('qmeta_file') ) {
        ch_qmeta_file = Channel.fromPath("${params.qmeta_file}", checkIfExists: true)
